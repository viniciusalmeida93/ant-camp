...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to organizer area to publish initial results.
        frame = context.pages[-1]
        ...
        # -> Click on 'Criar novo campeonato' button to create a new championship.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Real-time leaderboard update success').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The test plan execution failed because the leaderboard did not update dynamically in real-time after editing published results.")
