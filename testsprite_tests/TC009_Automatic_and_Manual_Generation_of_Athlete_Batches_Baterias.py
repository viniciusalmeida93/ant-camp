...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the organizer area to generate batches automatically for a championship.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click 'Criar novo campeonato' button to start creating a new championship
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Automatic batch generation successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Automatic batch generation did not distribute athletes based on categories as expected, and manual editing verification could not be completed.')
